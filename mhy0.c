...
/* data and routines also needed by mhy1 */
const uint8_t gf256exp[256] = {
...
/* TODO: CBT versions use different constants here */
const uint8_t lut[3][16] = {
...
static int mhy0_encrypt(uint8_t* in, int isHeader) {
	...
	// TODO Account for a data size below 0x21/header size below 0x39
	size_t size = isHeader ? 0x39 : 0x21;
	...
	uint8_t t[16];
	int done = 0;
	size_t processedSize = roundSize + 4;
	uint8_t* in2 = in + 4;
	while (!done && processedSize < size) {
		for (i = 0; i < chunkSize; i++) {
			in[i + processedSize] ^= in2[i];
			if (i + processedSize >= size - 1) {
				done = 1;
				break;
			}
		}
	...
	*(uint32_t*) in ^= *(uint32_t*) in2;
	for (i = 0; i < roundSize; i += 16) {
		for (j = 0; j < 3; j++) {
			for (k = 0; k < 16; k++) {
				v = lut[j][k];
				keyIdx = k % 8;
				q = key[keyIdx] ^ in2[i + k];
				for (l = 0; l < 256; l++) {
...
static int mhy0_decrypt(uint8_t* in, int isHeader) {
	if (in == NULL) {
		return -1;
	}
	// TODO Account for a data size below 0x21/header size below 0x39
	...
	for (i = 0; i < roundSize; i += 16) {
		for (j = 0; j < 3; j++) {
			for (k = 0; k < 16; k++) {
				...
				blkScrambleTblIdx |= gf256mul(gf_idx[keyIdx], in2[i + v]);
...
int extract_mhy0(uint8_t* in_buf, const char* filename, uint8_t** _next_mhy0) {
	...
	int ret = mhy0_decrypt(in_buf + 8, 1);
	if (ret) {
	...
	ret = LZ4_decompress_safe((const char*) (in_buf + 0x2f), (char*) hdr_buf, hdr_sz - 0x27, hdr_sz_dec);
	if (ret < 0) {
	...
	for (i = 0; i < blk_cnt; i++) {
		...
		ret = mhy0_decrypt(in_buf + data_off, 0);
		if (ret) {
	...
	fprintf(stderr, "total size of this mhy0 0x%08lx\n", total_sz);
	next_mhy0 = in_buf + total_sz; /* + ((total_sz + 3) & ~3)*/
