...
void xorCrypt(uint8_t* ibuf, size_t ilen, const uint8_t* kbuf, size_t klen) {
	unsigned int i;
	for (i = 0; i < ilen; i++) {
		ibuf[i] ^= kbuf[i % klen];
	}
...
static const uint8_t mul3Tbl[256] = {
...
static const uint8_t mul9tbl[256] = {
...
static const uint8_t mulBtbl[256] = {
...
static const uint8_t mulDtbl[256] = {
...
static const uint8_t mulEtbl[256] = {
...
static const uint8_t shiftTbl[16] = {
	0, 5, 10, 15, 4, 9, 14, 3, 8, 13, 2, 7, 12, 1, 6, 11
};

static const uint8_t shiftTblInv[16] = {
...
static void subBytes(uint8_t* key) {
	unsigned int i;
	for (i = 0; i < 16; i++) {
		key[i] = sbox[key[i]];
	}
...
static void subBytesInv(uint8_t* key) {
	unsigned int i;
	for (i = 0; i < 16; i++) {
		key[i] = sboxInv[key[i]];
	}
...
static void shiftRowsInv(uint8_t* key) {
	unsigned int i;
	char key2[16];
	memcpy(key2, key, 16);
	for (i = 0; i < 16; i++) {
		key[i] = key2[shiftTblInv[i]];
	}
}

static void mixCol(uint8_t* key) {
	uint8_t a0 = key[0];
	uint8_t a1 = key[1];
	uint8_t a2 = key[2];
	uint8_t a3 = key[3];
	key[0] = mul2Tbl[a0] ^ mul3Tbl[a1] ^ a2 ^ a3;
	key[1] = mul2Tbl[a1] ^ mul3Tbl[a2] ^ a3 ^ a0;
	key[2] = mul2Tbl[a2] ^ mul3Tbl[a3] ^ a0 ^ a1;
	key[3] = mul2Tbl[a3] ^ mul3Tbl[a0] ^ a1 ^ a2;
}

static void mixCols(uint8_t* _key) {
	uint8_t key[4][4];
	memcpy(key, _key, 16);
	unsigned int i;
	for (i = 0; i < 4; i++) {
...
static void mixColInv(uint8_t* key) {
	uint8_t a0 = key[0];
	uint8_t a1 = key[1];
	uint8_t a2 = key[2];
	uint8_t a3 = key[3];
...
static void mixColsInv(uint8_t* _key) {
	uint8_t key[4][4];
	memcpy(key, _key, 16);
	unsigned int i;
	for (i = 0; i < 4; i++) {
...
void aesScrambleKey(uint8_t* key, const uint8_t roundKeys[11][16]) {
	...
	for (i = 1; i < 10; i++) {
		...
		shiftRows(key);
		mixCols(key);
		xorCrypt(key, 16, roundKeys[i], 16);
	}
	subBytes(key);
	shiftRows(key);
	xorCrypt(key, 16, roundKeys[10], 16);
}
...
void aesUnscrambleKey(uint8_t* key, const uint8_t roundKeys[11][16]) {
	...
	subBytesInv(key);
	for (i = 1; i < 10; i++) {
		xorCrypt(key, 16, roundKeys[10 - i], 16);
		mixColsInv(key);
		shiftRowsInv(key);
		subBytesInv(key);
	}
...
/* These variants switch the normal and inverted processes. Used in ec2b and blk0 */
...
/* These variants do the XOR-round-key process differently. Used in blb3 */
...
void aesGetRoundKeys(const uint8_t* key, uint8_t roundKeys[11][16]) {
	...
	while (b < 176) {
		...
		a[0] ^= rcon[r];
		xorCrypt(a, 4, _roundKeys + b - 16, 4);
		memcpy(_roundKeys + b, a, 4);
		b += 4;
		for (i = 0; i < 3; i++) {
			memcpy(a, _roundKeys + b - 4, 4);
			xorCrypt(a, 4, _roundKeys + b - 16, 4);
...
void aesGetRoundKeysBlb3(const uint8_t* key, uint8_t roundKeys[11][16]) {
	...
	unsigned int o = 31;
	for (i = 1; i < 11; i++) {
		a = sbox[_roundKeys[o - 20]] ^ _roundKeys[o - 20];
		b = sbox[_roundKeys[o - 16]] ^ _roundKeys[o - 16];
		c = sbox[_roundKeys[o - 24]] ^ rcon[i] ^ _roundKeys[o - 31];
		d = sbox[_roundKeys[o - 28]] ^ _roundKeys[o - 28];
		e = 0;
		_roundKeys[o - 15] = c;
...
void genXorpadFromSeed(uint64_t seed, uint8_t* _xorpad, size_t sz, unsigned int reroll, unsigned int be) {
	if (_xorpad == NULL) return;
	unsigned int i;
	uint64_t r;
	uint64_t* xorpad = (uint64_t*) _xorpad;
	init_genrand64(seed);
	if (reroll) {
